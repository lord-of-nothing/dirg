...
QUuid Vertex::get_id() { return id; }

QString Vertex::get_name() { return name; }

QVector<QUuid> &Vertex::get_edges() { return edges; }

QVector<QUuid> &Vertex::get_polygons() { return polygons; }

void Vertex::add_edge(QUuid edge_id) { edges.append(edge_id); }

void Vertex::add_polygon(QUuid polygon_id) { polygons.append(polygon_id); }
...
Edge::Edge(QUuid start, QUuid finish, QString name, int property)
	: property(property), coords({start, finish}), name(name) {
...
QUuid Edge::get_id() { return id; }

QString Edge::get_name() { return name; }

QVector<QUuid> &Edge::get_polygons() { return polygons; }

QPair<QUuid, QUuid> Edge::get_coords() { return coords; }

void Edge::add_polygon(QUuid polygon_id) { polygons.append(polygon_id); }

int Edge::get_property() { return property; }
...
QUuid Polygon::get_id() { return id; }

QString Polygon::get_name() { return name; }

int Polygon::get_polygons_total() { return total_polygon_number; }

int Polygon::get_number() { return cur_polygon_number; }

QVector<QUuid> &Polygon::get_vertices() { return vertices; }

QVector<QUuid> &Polygon::get_edges() { return edges; }
...
bool check_new_point(double first_x, double first_y, double second_x,
	...
	//     double old_first_x =
	//     all_vertices[pair.second.get_coords().first].get_x(); double
	//     old_first_y = all_vertices[pair.second.get_coords().first].get_y();
	//     double old_second_x =
	//     all_vertices[pair.second.get_coords().second].get_x(); double
	//     old_second_y = all_vertices[pair.second.get_coords().second].get_y();
	//     if (check_intersect(old_first_x, old_first_y, old_second_x,
	//     old_second_y, second_x, second_y, third_x, third_y) == false) {
	//         second_check = false;
	//     }
	// }
	...
	for (auto pair = all_edges.begin(); pair != all_edges.end(); ++pair) {
		double old_first_x =
			all_vertices[pair.value().get_coords().first].get_x();
		double old_first_y =
			all_vertices[pair.value().get_coords().first].get_y();
		double old_second_x =
			all_vertices[pair.value().get_coords().second].get_x();
		double old_second_y =
			all_vertices[pair.value().get_coords().second].get_y();
		if (check_intersect(old_first_x, old_first_y, old_second_x,
...
bool point_in_polygon(double x, double y, QUuid polygon_id) {
	...
	for (auto &edge : all_polygons[polygon_id].get_edges()) {
		if (check_intersect(
				...
				all_vertices[all_edges[edge].get_coords().first].get_x(),
				all_vertices[all_edges[edge].get_coords().first].get_y(),
				all_vertices[all_edges[edge].get_coords().second].get_x(),
				all_vertices[all_edges[edge].get_coords().second].get_y())) {
			count_intersection++;
		}
	}
...
bool isNotIntersecting(QVector<std::pair<double, double>> vertices) {
	for (int i = 0; i < vertices.size(); i++) {
		...
		//     all_vertices[pair.second.get_coords().first].get_x(); double
		//     old_first_y =
		//     all_vertices[pair.second.get_coords().first].get_y(); double
		//     old_second_x =
		//     all_vertices[pair.second.get_coords().second].get_x(); double
		//     old_second_y =
		//     all_vertices[pair.second.get_coords().second].get_y(); if
		//     (check_intersect(old_first_x, old_first_y, old_second_x,
		//     old_second_y, vertices[i].first, vertices[i].second, vertices[(i
		//     + 1) % vertices.size()].first, vertices[(i + 1) %
		//     vertices.size()].second)) {
		//         return false;
		//     }
		// }
